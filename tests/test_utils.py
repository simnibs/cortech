...
def test_compute_sphere_radius(frac):
    ...
    radius_frac = cortech.utils.compute_sphere_radius(
        frac, thickness, radius, radius_cubed
    )
    ...
    np.testing.assert_allclose(
        (1 - frac) * (v_frac - v_inner), frac * (v_outer - v_frac)
    )
...
def test_k_ring_neighbors_single(k, diamond_vertices, diamond_adjacency_matrix):
    ...
    knn, kr = cortech.utils.k_ring_neighbors(
        k, indices, len(diamond_vertices), A.indices, A.indptr
    )
